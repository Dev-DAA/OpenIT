...
uint8_t Field::GetPosX() const // Возвращаем текущее положение курсора.
{
    return m_coordinates.x;
}

uint8_t Field::GetPosY() const // Возвращаем текущее положение курсора.
{
...
int16_t Field::Open() // Открытие ячейки, получение её содержимого, выставление статуса "открыта".
{
    uint8_t index = GetIndex(m_coordinates.x,m_coordinates.y);
    int16_t value = m_playground[index];
