...
void Field::Move(
    OpenIt::Direction direction) // Перемещение по горизонтали/вертикали в зависимости от действующего игрока.
{
    if (direction == OpenIt::Direction::LEFT && m_coordinates.x > 0)
    {
        m_coordinates.x -= 1;
    }
    if (direction == OpenIt::Direction::RIGHT && m_coordinates.x < m_length)
    {
        m_coordinates.x += 1;
    }
    if (direction == OpenIt::Direction::UP && m_coordinates.y > 0)
    {
        m_coordinates.y -= 1;
    }
    if (direction == OpenIt::Direction::DOWN && m_coordinates.y < m_height)
    {
        m_coordinates.y += 1;
    }
}
