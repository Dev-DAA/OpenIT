...
OpenIt::Action IO::GetAction() // Получаем действие для выполнения при нажатии клавиши.
{
        switch ()
        {
            case (GetAsyncKeyState(0x41) && 0x41):
        ...
        if(GetAsyncKeyState(0x41) && 0x41)
            return OpenIt::Action::LEFT;
        else if(GetAsyncKeyState(0x44) && 0x44)
            return OpenIt::Action::RIGHT;
        else if(GetAsyncKeyState(0x57) && 0x57)
            return OpenIt::Action::UP;
        else if(GetAsyncKeyState(0x53) && 0x53)
            return OpenIt::Action::DOWN;
        else if(GetAsyncKeyState(0x4F) && 0x4F)
            return OpenIt::Action::OPENCELL;
        else if(GetAsyncKeyState(0x4E) && 0x4E)
            return OpenIt::Action::NEWGAME;
        else return OpenIt::Action::STAY;  
}
...
void IO::Render(const Field& obj, const Player* player)
{
    ...
    {
        std::wcout << std::left << std::setw(3 * (FIELD_LENGTH / 2)) << "1: " << player[0].GetScore()
                   << std::setw(7 * FIELD_LENGTH - 8) << " " << std::left << std::setw(3 * (FIELD_LENGTH / 2))
                   << "2: " << player[1].GetScore() << '\n';
    }
    else if (length < 5 && length > 2)
    {
        std::wcout << std::left << std::setw(5 * (FIELD_LENGTH / 3)) << "P 1: " << player[0].GetScore()
                   << std::setw(7 * FIELD_LENGTH - 12) << " " << std::left << std::setw(5 * (FIELD_LENGTH / 3))
                   << "P 2: " << player[1].GetScore() << '\n';
    }
    else
    {
        std::wcout << std::left << std::setw(10 * (FIELD_LENGTH / 5)) << "Player 1: " << player[0].GetScore()
                   << std::setw(7 * FIELD_LENGTH - 22) << " " << std::left << std::setw(10 * (FIELD_LENGTH / 5))
